---
id: FK-03
slug: evals-launch-gate
name: Evals as a Launch Gate
thesis: "\"It seems to work\" is not a deliverable."
kills: "How do we know it actually works?"
taxonomy: eval discipline (precondition for the evaluator-optimizer workflow)
description: >
  Use on every engagement, before code is written, when anyone asks how they
  will know the system actually works — defining metrics, building labeled
  fixture sets, choosing deterministic scorers vs. LLM-as-judge, and gating
  launch and releases on measured results instead of demo impressions.
pairs_with_all: true
triggers:
  - eval
  - metric
  ...
  - judge
exemplars:
  - name: Verity's CI harness
    url: https://verity-navy-five.vercel.app
  - name: Prospectus's published eval report
    url: https://prospectus-nickarndt.vercel.app
grounded_in:
  - source: Define success criteria and build evaluations (Claude Docs)
    authors: Anthropic
    url: https://platform.claude.com/docs/en/test-and-evaluate/develop-tests
    principle: >
      Define specific, measurable success criteria before building; automate
      grading when possible (multiple-choice, string match, code-graded,
      LLM-graded), using simple deterministic checks where answers are
      clear-cut and LLM-based scales for nuanced qualities that resist
      traditional metrics.
  - source: Building effective agents
    ...
    principle: >
      The evaluator-optimizer workflow — one model call generates while
      another provides evaluation and feedback in a loop — is particularly
      effective when clear evaluation criteria exist and iterative
      refinement provides measurable value.
---

<!-- `pairs_with_all: true` is a rubric policy flag, not decoration:
     recommend_pattern appends FK-03 to any non-empty recommendation, because
     this pattern's own "when not to use" is "never." The policy lives here in
     data — not hardcoded in the tool — so the rubric stays generic and the
     pattern file remains the single source of truth about itself. -->
