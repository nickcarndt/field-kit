...
name: Grounded Retrieval with Abstention
thesis: A system that says "I don't know" is more trustworthy than one that always answers.
kills: "Hallucination risk makes this unusable in our industry."
taxonomy: augmented LLM (retrieval workflow)
description: >
  Use when a client needs question-answering over a real document corpus and
  provenance decides whether the answer is usable — filings, research archives,
  regulatory and legal documents — and the blocking objection is hallucination
  risk in a domain where a confidently wrong answer is worse than no answer.
triggers:
  - retriev
  - document
  - filing
...
exemplars:
  - name: Prospectus
    url: https://prospectus-nickarndt.vercel.app
grounded_in:
  - source: Introducing Contextual Retrieval
    ...
    principle: >
      Combine embedding search with BM25 lexical matching and rerank the
      fused results — the post's conclusions endorse hybrid over
      embeddings-only and reranking over none; its headline failure-rate
      numbers are measured with contextual chunk annotation added, a layer
      this pattern omits.
  - source: Introducing Citations on the Anthropic API
    authors: Anthropic
    date: "2025"
    url: https://claude.com/blog/introducing-citations-api
    principle: >
      Grounding is a first-class API capability — Claude cites specific
      passages from a provided document set, so provenance is produced by the
      platform, not requested in a prompt.
  - source: Structured outputs (Claude Docs)
    authors: Anthropic
    url: https://platform.claude.com/docs/en/build-with-claude/structured-outputs
    principle: >
      Constrained decoding guarantees schema-conforming responses — the
      mechanism that makes answers, citations, and abstention machine-checkable
      instead of parse-and-hope.
---

<!-- Same machine contract as FK-01. `date` inside grounded_in is optional:
     year-only where the source page's displayed date is ambiguous (the
     citations post was re-stamped in a blog migration), absent for living
     docs pages. catalog.py must not require it. -->
